...
That said, the table is quite boring. The fun of going through GPX file hell is in the visualisation that follows. I haven't spent too long on this yet, so there's plenty more fun to be had. Before getting into the `ggplot2` chunks, I tidy up the summary table by renaming some columns and pivoting everything to long format. The pivot makes visualisation _way easier_ later on.
...
Then we can get mapping. I do a small wrangle beforehand because at some point I might add activity statistics to each individual map.
...
We can also, with a little effort, create an interactive map to better replicate the behaviour of the Strava app maps. You can view the output of the below codechunk [here](https://rpubs.com/langton_/fstrava_leaflet).
